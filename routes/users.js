...
router.route('/dashboard')
    ...
    .get(ensureAuthenticated, function(req, res, next){
        ...
        fileModel.find({}).limit(20).sort({date: -1}).exec(function(err, files){
            if (err){
            ...
            folderModel.find({}).limit(20).sort({date: -1}).exec(function(err, folders){
                ...
                if(folders){
                    res.render('dashboard', {folders:folders, files: files, avatar: avatar});
                }
            })
        });
    })
    ...
    .get(ensureAuthenticated, function(req, res, next){
        ...
        fileModel.find(query, function(err, files){
            ...
            folderModel.find(query, function(err, folders){
                ...
                res.render('collection', {'files': files, 'folders': folders, avatar: avatar});
            });
        });
    })
;
